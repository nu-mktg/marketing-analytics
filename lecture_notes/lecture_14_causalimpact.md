...
**Q3.** A MAPE of 22% in the pre-period means the fitted model misses the observed outcome by about 22% in an average pre-period week — and it does so on the very data it was trained on, which is where the fit should be at its best. Note what MAPE is *not*: it is a mean absolute percentage **error**, not a share of variance explained. $100\% - \text{MAPE}$ is not $R^2$, and the two need not move together. The post-period counterfactual is therefore unreliable — it will have very wide credible intervals. The first step: examine whether different or additional control covariates improve the pre-period fit. If no good covariates exist, CausalImpact is not the right tool for this setting.
