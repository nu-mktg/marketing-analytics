...
> **Also asked on the slides:** *"P(alive) = 0.12. CLV₁₂ = \$340. What is the **expected** value of this customer?"* — Roughly P(alive) × CLV = 0.12 × \$340 = **\$40.80**. The \$340 is what the customer is worth *if still active*; you only collect it if they are, so the decision-relevant figure discounts it by the probability they are alive. Part C of the worked example uses exactly this rule to justify retention spend ("the expected return is approximately P(alive) × CLV"), and this customer is the Marcus case: a large headline CLV attached to a P(alive) near zero. Spending \$50 to retain them would be a loss even though CLV₁₂ is \$340.
